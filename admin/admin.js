(function () {
  ...
  (function(){
    ...
    btn.addEventListener('click', function(){
      ...
      var preset = (document.querySelector('.svv-b-preset') && document.querySelector('.svv-b-preset').value) || '';
      var mapengine = (document.querySelector('.svv-b-mapengine') && document.querySelector('.svv-b-mapengine').value) || 'auto';
      ...
      var hourly = document.querySelector('.svv-b-hourly') && document.querySelector('.svv-b-hourly').checked ? '1' : '0';
      var hoursEl = document.querySelector('.svv-b-hours');
      var hours = hoursEl ? Math.max(3, Math.min(24, parseInt(hoursEl.value || '24', 10) || 24)) : 24;
      var tides = document.querySelector('.svv-b-tides') && document.querySelector('.svv-b-tides').checked ? true : false;
      ...
      if (preset) sc += ' preset="' + preset + '"';
      if (map === '1') sc += ' map="1"';
      if (mapengine && mapengine !== 'auto') sc += ' map_engine="' + mapengine + '"';
      if (animate === '0') sc += ' animate="0"';
      if (hourly === '1') sc += ' hourly="1" hours="' + hours + '"';
      if (provs && provs !== 'openmeteo,smhi,yr') sc += ' providers="' + provs + '"';
      if (windunit) sc += ' wind_unit="' + windunit + '"';
      var extras = [];
      if (tides) extras.push('tides');
